...
function UserCardUI({ user, industry, bio, skills, latitude, longitude, currentUser, SwipeLeft, SwipeRight }) {
  ...
  useEffect(() => {
    function onKeyPressed(e) {
      ...
      else if (e.key === 'ArrowLeft') SwipeLeft?.();
      cooldownRef.current = true;
      setTimeout(() => { cooldownRef.current = false; }, 200);  // 200ms cooldown between key presses
    }
  ...
  return (
    <div className="card" onMouseDown={onMouseDown} style={{
      ...
      <h1>{user}</h1>
      <h3>{industry}</h3>
      {distance && <p>{distance} km away</p>}   {/* only shown if both users have coordinates */}
