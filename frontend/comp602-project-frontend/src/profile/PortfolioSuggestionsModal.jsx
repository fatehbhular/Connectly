  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError("Could not load suggestions. Try again later.");
      } finally {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {error && (
            <p style={{ margin: 0, fontSize: 13, color: "#C4785A", textAlign: "center" }}>
              {error}
            </p>
          )}
          ...
          {!loading && !error && data && (
            <p style={{ margin: 0, fontSize: 10, color: "#B0A99F", textAlign: "center" }}>
              {data.source === "ai" ? "Personalised with AI" : "Curated for your industry"}
            </p>
          )}
